md_directory <- fe.module_dir;
md_dir <- md_directory;

class ModernDialog
{
	fe = ::fe
	md_dir = FeConfigDirectory + "themes/HeyChromey/scripts/modern-dialogs/";
	frame = null
	title = null
	list = null
	list_shader = null
	...
	constructor ( _x, _y, _w, _rh, _r )
	{
		...
		list_shader = fe.add_shader( Shader.VertexAndFragment, md_dir + "modern_dialog_listbox.vert", md_dir + "modern_dialog_listbox.frag" )
		list.shader = list_shader
		list_shader.set_param( "screenSize", fe.layout.width, fe.layout.height )

		title_shader = fe.add_shader( Shader.VertexAndFragment, md_dir + "modern_dialog_listbox.vert", md_dir + "modern_dialog_listbox.frag" )
...
class ModernDialogFrame
{
	...
	constructor ( _x, _y, _w, _rh, _r )
	{
		...
		image = fe.add_image( md_dir + "frame512x512pm.png", _x , _y, frame_height + frame_scaled, 0 )
		image.zorder = 3
		shader = fe.add_shader( Shader.VertexAndFragment, md_dir + "modern_dialog_frame.vert", md_dir + "modern_dialog_frame.frag" )
		image.shader = shader
		image.mipmap = true

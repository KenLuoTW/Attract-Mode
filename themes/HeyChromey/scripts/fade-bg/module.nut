...
class FadeBG
{
	...
	//
	constructor( label, x, y, width, height, artpath, target=::fe )
	{
	...
	function get_artwork_file()
	{
		local supported_img_extensions = [ "png", "jpg", "jpeg", "gif", "bmp", "tga", "mp4" ];
		local name = fe.game_info(Info.Name, index_offset);
		...
		switch(_label)
		{
			case "bg":
				return _label;
			case "screenshot":
				path = _artpath + "/screenshot/" + name
				break;
			case "snap":
				path = _artpath + "/videos/" + name
				break;
			case "fanart":
			case "auto":
				path = _artpath + "/fanart/" + name
				break;
			default:
				return _label;
				break;
	...
	function on_tick( ttime )
	{
			...
			((interval>0)&&(ttime-_last_switch>interval)) )
		{
			...
			local tmp = fe.get_art( get_artwork_file(),
					index_offset,
					filter_offset,
					Art.FullList );
